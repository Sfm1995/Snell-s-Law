...
vector<double> get_mp_t_0(licht Licht, surface Surface) {
	...
	if (dot(n, v) == 0) {
		...
		if (v[0]) {
			x = (dot(n, p) * v[0] + (v[1] * n[1] + v[2] * n[2]) * s[0] - (n[1] * s[1] + n[2] * s[2]) * v[0]) / dot(n, v);
			y = (x - s[0]) * v[1] / v[0] + s[1];
			z = (x - s[1]) * v[2] / v[0] + s[2];
			t_0 = (x - s[0]) / v[0];
			...
			x = s[0];
			y = s[1];
			z = (n[0] * (x - p[0]) + n[1] * (y - p[1])) / n[2] + p[2];
			t_0 = (z - s[2]) / v[2];
		}
		else if (!v[0] && v[1] && !v[2]) {
			x = s[0];
			z = s[2];
			y = (n[0] * (x - p[0]) + n[2] * (z - p[2])) / n[1] + p[1];
			t_0 = (y - s[1]) / v[1];
		}
		else if (!v[0] && v[1] && v[2]) {
			x = s[0];
			y = (n[0] * (p[0] - x) * v[1] + n[1] * p[1] * v[1] + v[2] * s[1] * n[2] - n[2] * s[2] * v[1] + n[2] * p[2] * v[1]) / dot(n, v);
			z = v[2] * (y - s[1]) / v[1] + s[2];
			t_0 = (y - s[1]) / v[1];
		}
		mp_t_0 = { x,y,z,t_0 };
